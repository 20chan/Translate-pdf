...
```
이 프로그램은 기존의 언어에서는 나누어 떨어지지 않는다. 우리는 이거을 지연 평가를 이용하여 더 모듈화하고 파트가 다른 곳에 사용되는 것을 보여줄 것이다.

뉴턴-랩슨 알고리즘이 계속하여 근사값들을 구하므로 이것을 명시적인 근사값 리스트로 구하는것이 자연스럽다. 각각의 근사값은 이전의 값으로부터 다음 함수를 사용하여 구해진다.
```
next N x = (x + N/x) / 2
```
`next N`은 이전 근사값을 다음 근사값으로 바꿔주는 함수이다. 이 함수를 `f`라 하면, 근사값들의 시퀀스는
```
[a0, f a0, f(f a0), f(f(f a0)), ..]
```
이고 이 값을 계산하는 함수를 정의하면 다음과 같다.
```
repeat f a = cons a (repeat f (f a))
```
그래서 근사값들의 리스트는 다음으로 구할 수 있다.
```
repeat (next N) a0
```
`repeat`은 무한한 출력을 가진 함수의 예이다. 하지만 프로그램이 원하지 않는 나머지 값들은 계산이 되지 않을테니 문제가 되지 않는다. 무한한 출력이 가능하다는 것은, 몇번째의 근사값도 필요하다면 제한이 없는 반복으로 계산할 수 있다는 것이다.

제곱근의 값을 구하는 다른 함수는 `within` 허용 오차와 근사값의 리스트를 받아 첫 두 개의 근가값이 허용 오차보다 작은지 확인하고, 그렇지 않다면 다음을 검사하여 내려가는 함수이다. 이것은 다음과 같이 정의될 수 있다.
```
within eps (cons a (cons b res)) = 
        = b, if abs(a-b) <= eps
        = within eps (cons b res), otherwise
```
모든 파트들을 합치면 다음 모양이 된다.`
```
...
우리는 제곱근의 근사값의 시퀀스를 재사용해야 한다. 당연히, `within`과 `relative`를 어떠한 근사값의 시퀀스를 생성하는 수치 알고리즘에서 사용하는 것은 가능하다. 그래서 우리는 이것을 수치 미분 알고리즘에서 사용할 것이다.
점에서의 함수를 미분한것의 결과는 그 점에서의 함수의 그래프의 기울기이다. 이것은 함수에서 주어진 점과 그 점과 매우 가까이 있는 다른 점 사이의 선의 기울기를 구함으로써 꽤 간단하게 추정할 수 있다. 이것은 두 점이 충분히 가까이 있다면 함수의 그래프가 그 구간에서 많이 꺾이지 않는다는 가정 하에 있다. 이것은 다음의 정의로 구현된다.
```
easydiff f x h = (f(x+h)-f x) / h
```
적절한 `h`의 값은 매우 작아야 할 것이다. 아쉽게도, `h`가 너무 작다면 두 값 `f(x+h)`와 `f(x)`는 매우 가까워져 뺄셈에서 반올림 오류가 일어나 결과를 망칠 수 있다. 그렇다면 적당한 `h`의 값은 어떻게 정할까? 이 딜레마의 해결책중 하나는 `h`의 값을 적당하게 큰 값부터 시작하여 점차 작게 해가면서 근사값의 시퀀스를 계산하는 것이다. 시퀀스는 미분된 값에 가까워져야 하지만 반올림 오류로 부정확해질 것이다. 만약 `within eps`가 충분히 정확한 근사값을 고르는데 사용된다면 반올림 오류가 결과에 미치는 영향이 더 줄어든다. 우리는 시퀀스를 계산할 함수가 필요하다.
```
differentiate h0 f x = map (easydiff f x) (repeat halve h0)
halve x = x/2
```
여기서 `h0`은 `h`의 초기 값이고, 연속적인 값들은 계속해서 반을 함으로써 얻어진다. 이 함수를 이용하면, 어떤 점에서 미분은
```
within eps (differentiate h0 f x)
```
로 계산할 수 있다. 이 해결책도 엄청 만족적이진 않다. 근사값의 시퀀스가 매우 느리게 수렴하기 때문이다. 아주 간단한 수학으로 이를 도울 수 있다. 시퀀스의 원소들은 다음으로 나타낼 수 있다.
```
정답 + h를 포함한 오류항
```
이론적으로 오류항이 `h`의 거듭제곱에 비례함으로, `h`가 작아질수록 에러도 작아진다. 정답을 `A`라고 하면, 에러항은 `B*h**n`이 될 것이다. 각 근사값은 다음에서 쓰이는 `h`보다 두배 큰 값이 사용되니까, 아무 두 연속한 근사값은 다음처럼 나타낼 수 있다.
```
a(i)   = A + B*(2**n)*(h**n)
a(i+1) = A + B*(h**n)
```
이제 에러항은 사라진다. 우리는 결론적으로 다음 식을 얻었다.
```
    a(i+1)*(2**n) - a(i)
A = --------------------
          2**n - 1
```
당연히, 에러항도 대략 `h`의 거듭제곱이라는 근사값이라 이것도 정확한 값은 아니지만, 이건 더 정확한 근사값이다. 이 차이는 모든 연속적인 근사값의 쌍에 다음 함수를 적용시킬 수 있게 한다.
```
elimerror n (cons a (cons b rest)) = cons ((b*(2**n)-a)/(2**n-1)) (elimerror n (cons b rest))
```
근사값의 시퀀스에서 에러항을 제거함으로써 더 빠르게 수렴하는 시퀀스를 얻을 수 있다.

`elimerror`을 사용해도 문제 하나가 남는다. 우리는 `n`의 정확한 값을 알아야 한다. 보통 이것은 예측하기 어렵지만 계산해내기는 쉽다. 다음 함수가 이를 정확하게 구해내는 것을 보여주는 것은 어렵지 않으니 증명을 여기서 보여주지는 않겠다.
```
order (cons a (cons b (cons c rest))) = round (log2 (a-c)/(b-c)-1)
round x = x rounded to the nearest integer
log2 x = the logarithm of x to the base 2
```
이제 더 괜찮은 근사값의 시퀀스를 위한 함수는 다음으로 정의된다.
```
improve s = elimerror (order s) s
```
함수 `f`의 미분은 `improve`를 사용하여 더 효과적으로 계산할 수 있다.
```
within eps (improve (differentiate h0 f x))
```
`improve`는 각 값이 계속 1/2되는 매개변수 `h`을 사용하여 계산된 근사값들의 시퀀스에서만 작동한다. 그런데 만약 이게 어떤 시퀀스를 입력으로 받는다면, 이는 결과값도 어떤 시퀀스가 된다! 이것으로 추정값들의 시퀀스는 한번 더 발전될 수 있다. 다른 에러항은 그때그때 제거되고 결과 시퀀스는 더더욱 빠르게 수렴한다. 그래서 다음은 매우 효율적으로 미분을 계산한다.
```
within eps (improve (improve (improve (differentiate h0 f x))))
```
수치 분석 용어에서 이것은 `fourth order method`과 비슷하고 이 방법은 결과값을 매우 빠르게 구한다. 심지어 다음과 같이 정의할 수 있다.
```
super s = map second (repeat improve s)
second (cons a (cons b rest)) = b
```
이는 더더욱 향상된 추정값들의 시퀀스들의 시퀀스를 얻기 위해 `repeat improve`를 사용하고 새로운 추정값들의 시퀀스를 향상된 시퀀스들중 두번째 값만으로 이루어진 새로운 추정값들의 시퀀스를 구성한다. (이것은 두번째 값이 가져오기 최고의 값임을 나타낸다. 이는 첫번째보다 더 정확하고 더 많이 계산할 필요도 없다). 이 알고리즘은 매우 복잡하다. 이는 더더욱 나은 수치적 방법을 사용하여 더 많은 추정값을 계산한다. 결론적으로 미분을 매우 효율적으로 계산하는 프로그램은 다음과 같다.
```
within eps (super (differentiate h0 f x))
```
이것은 문재를 해결하는데 오버한 감이 없잖아 있지만, 요점은 `super`처럼 복잡한 알고리즘도 `lazy evaluation`을 사용한 모듈화된다면 쉽게 표현된다는 것이다.
